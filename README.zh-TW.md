...
- 使用者餘額查詢
- 呼叫 IP 來源與權杖洩漏風險分析：依權杖彙整 IP/網段、5 分鐘內多 IP 並用及相較前一週期的新 IP，並提供可解釋的風險評分（僅作為排查線索）；同時統計峰值請求/分鐘、活躍分鐘數與最繁忙分鐘
...
- 資料庫帳號建議使用唯讀權限。
- 權杖安全分析中的 IP 來源訊號依賴 `logs.ip`，每分鐘請求統計則僅依賴消費日誌與呼叫時間。如頁面顯示「未採集到 IP」，請在 new-api 的「個人設定 → 通知設定」中為相關使用者啟用「記錄 IP 位址」；該設定預設關閉，歷史日誌無法補登。反向代理部署還需正確設定 new-api 的 `TRUSTED_PROXIES` 與 `X-Forwarded-For`。
