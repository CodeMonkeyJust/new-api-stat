...
- 修复浏览器无法登录：XSRF-TOKEN Cookie 的 Path 由上下文路径 `/newapi-stat-api` 改为 `/`，使位于 `/` 的前端 SPA 页面（Vite 开发服务器/反向代理部署）能通过 document.cookie 读取 token 并自动附带 X-XSRF-TOKEN 请求头，避免登录等写操作被 CSRF 拦截返回 403。
