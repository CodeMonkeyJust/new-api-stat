...
- 用户余额查询
- 调用 IP 来源与令牌泄露风险分析：按令牌汇总 IP/网段、5 分钟多 IP 并发和上一周期新 IP，并给出可解释的风险评分（仅作为排查线索）；同时统计峰值请求/分钟、活跃分钟数和最繁忙分钟
...
- 数据库账号建议使用只读权限。
- 令牌安全分析中的 IP 来源信号依赖 `logs.ip`，每分钟请求统计则仅依赖消费日志和调用时间。如页面显示“未采集到 IP”，请在 new-api 的“个人设置 → 通知设置”中为相关用户启用“记录 IP 地址”；该设置默认关闭，历史日志无法补录。反向代理部署还需正确配置 new-api 的 `TRUSTED_PROXIES` 和 `X-Forwarded-For`。
