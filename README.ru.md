...
- Анализ IP-источников и риска утечки токенов: агрегирует IP/сети по токену, выявляет активность с нескольких IP за 5 минут и новые IP относительно предыдущего периода, рассчитывает объяснимую оценку риска (только сигнал для проверки)
...
- Анализ безопасности токенов зависит от `logs.ip`. Если страница сообщает об отсутствии IP-данных, включите «Record IP Address» для нужных пользователей в new-api в разделе «Профиль → Настройки уведомлений». По умолчанию этот параметр отключён, а исторические журналы нельзя заполнить задним числом. При развёртывании за обратным прокси также правильно настройте `TRUSTED_PROXIES` и `X-Forwarded-For` в new-api.
