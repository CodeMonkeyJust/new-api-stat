...
- ユーザー残高の照会
- 呼び出し元 IP とトークン漏えいリスク分析：トークン単位で IP/ネットワークを集計し、5 分以内の複数 IP 利用や前期間からの新規 IP を検出して、説明可能なリスクスコアを表示（調査の手がかりであり、漏えいの断定ではありません）
...
- トークンセキュリティ分析は `logs.ip` に依存します。ページに IP データがないと表示された場合は、new-api の「プロフィール → 通知設定」で対象ユーザーの「Record IP Address」を有効にしてください。この設定はデフォルトで無効で、過去のログへ遡って記録することはできません。リバースプロキシ環境では、new-api の `TRUSTED_PROXIES` と `X-Forwarded-For` も正しく設定してください。
